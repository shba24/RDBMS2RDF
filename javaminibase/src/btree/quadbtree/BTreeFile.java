...
public class BTreeFile extends IndexFile
  ...
  public BTreeFile(String filename)
     ...
     * - headerPage, headerPageId vaqid and pinned
  ...
  private void updateHeader(PageId newRoot)
    ...
    // ASSERTIONS:
    // - headerPage, headerPageId vaqid, pinned and marked as dirty

  }

  /**
   * insert record with the given key and qid
   *
   * @param key the key of the record. Input parameter.
   * @param qid the qid of the record. Input parameter.
  ...
  public void insert(KeyClass key, QID qid)
    ...
    if (headerPage.get_rootId().pid == INVALID_PAGE) {
      ...
      // ASSERTIONS:
      // - newRootPage, newRootPageId vaqid and pinned
    ...
    // ASSERTIONS:
    // - headerPageId, headerPage vaqid and pinned
    ...
    //                            information on headerpage is still vaqid
  ...
  private KeyDataEntry _insert(
      KeyClass key, QID qid,
    ...
    if (currentPage.getType() == NodeType.INDEX) {
      BTIndexPage currentIndexPage = new BTIndexPage(
          page,
      ...
      currentIndexPage = new BTIndexPage(
          pinPage(currentPageId),
          headerPage.get_keyType());

      // ASSERTIONS:
      // - upEntry != null
      // - currentIndexPage, currentIndexPageId vaqid and pinned
      ...
      // ASSERTIONS:
      // - newIndexPage, newIndexPageId vaqid and pinned
      // - currentIndexPage, currentIndexPageId vaqid and pinned
      ...
      // ASSERTIONS:
      // - currentLeafPage, currentLeafPageId vaqid and pinned
      ...
      if (rightPageId.pid != INVALID_PAGE) {
        ...
        // ASSERTIONS:
        // - newLeafPage, newLeafPageId vaqid and pinned
        // - currentLeafPage, currentLeafPageId vaqid and pinned
      }
      ...
      // ASSERTIONS:
      // - no pages pinned
      // - upEntry holds the vaqid KeyDataEntry which is to be inserted
   ...
   * lo_key is null.
   *
   * Starting record returned in *pstartqid, on page *pppage, which is pinned.
   *
  ...
  BTLeafPage findRunStart(
    ...
    if (pageno.pid == INVALID_PAGE) {        // no pages in the BTREE
      ...
      // startqid =INVALID_PAGEID ;             // the caller
      return pageLeaf;
    }
    ...
    // - pageno and sortPage vaqid and pinned
    ...
    // - pageLeaf, pageno vaqid and pinned
  ...
  private boolean NaiveDelete(KeyClass key, QID qid)
    ...
    while (true) {
      ...
      while (entry == null) { // have to go right
        ...
        System.out.println("nextpage.pid = " + nextpage.pid);

        leafPage = new BTLeafPage(
            pinPage(nextpage),
            headerPage.get_keyType());
     ...
     * We should have got true back from delUserQid above.  Apparently
  ...
  /*
   * Status BTreeFile::FullDelete (const void *key, const QID qid)
   *
   * Remove specified data entry (<key, qid>) from an index.
   *
  ...
  private KeyClass _Delete(
      ...
      while ((tmpEntry != null) && (BT.keyCompare(key, tmpEntry.key) >= 0)) {
        ...
        if (leafPage.delEntry(new KeyDataEntry(key, qid))) {
          ...
          if ((4 + leafPage.available_space()) <=
            ...
            siblingPage = new BTLeafPage(
                pinPage(siblingPageId),
                headerPage.get_keyType());
    ...
    if (sortPage.getType() == NodeType.INDEX) {
      ...
      if ((4 /*sizeof slot*/ + indexPage.available_space()) <=
        ...
        int direction;
        direction = parentPage.getSibling(
            key,
            siblingPageId);
        if (direction == 0) {
          // there is no sibling. nothing can be done.
        ...
        if (siblingPage.redistribute(indexPage, parentPage,
          ...
          leftChild.insertKey(
              parentPage.findKey(oldChildEntry),
